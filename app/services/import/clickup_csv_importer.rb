  ...
  class ClickupCsvImporter
    ...
    private
      ...
      def import_task(row)
        ...
        # Skip Fireflies Autogenerated tasks
        list_name = row["List Name"] || row[" List Name"]
        if list_name.present? && list_name.downcase.include?("fireflies") && list_name.downcase.include?("autogenerated")
          Rails.logger.debug "Skipping Fireflies Autogenerated task #{task_id}"
          return false
        end
        ...
        # Get list name for label (already extracted above, but re-extract here for consistency)
        list_name = row["List Name"] || row[" List Name"]
